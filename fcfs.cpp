...
// waiting time for first process is e
...
for (int i=0; i < n; i++) {
cout << proc[i].pid << "\t" << proc[i].arrival_time << "\t\t" << proc[i].burst_time << "\t\t" << proc[i].completion_time << "\t\t" << proc[i].turnaround_time << "\t\t" << proc[i].waiting_time << "\n";
}   
}
int main() {
    int n=5;
Process proc[] = {{1, 0, 5}, {2, 1, 3}, {3, 2, 2}, {4, 3, 4}, {5, 4, 1}};
findFCFS(proc, n);
